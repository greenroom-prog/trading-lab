...
def numbers(t):
    return set(re.findall(r"\d[\d,]*\.?\d*", t.replace(",", "")))


# Direction words. Two contracts on the SAME event pointing OPPOSITE ways
# share almost every keyword ("Fed", "September", "25", "rates") and so
# score high on any bag-of-words measure. v1 paired "no change in Fed
# rates" with "Fed CUT 25bps" at 0.41 confidence and reported a 52c
# "gap" that was simply two different questions. Polarity is checked
# before similarity, and a mismatch is a hard reject, not a penalty.
POLARITY = {
    "up":     {"above", "over", "higher", "increase", "increases", "hike",
               "hikes", "rise", "rises", "up", "greater", "exceed", "exceeds",
               "more", "gain", "gains"},
    "down":   {"below", "under", "lower", "decrease", "decreases", "cut",
               "cuts", "fall", "falls", "down", "less", "drop", "drops",
               "decline", "declines"},
    "flat":   {"unchanged", "no-change", "nochange", "hold", "holds",
               "steady", "same", "maintain", "maintains"},
    "yes":    {"win", "wins", "release", "releases", "announce", "announces",
               "complete", "completes", "reach", "reaches"},
    "negate": {"not", "no", "never", "fail", "fails", "without"},
}


def polarity(t):
    """Direction classes present in a title, plus whether it is negated."""
    w = set(re.findall(r"[a-z]+", t.lower()))
    # 'no change' / 'no increase' are FLAT, not negation
    flat = bool(re.search(r"\bno\s+(change|increase|decrease|hike|cut|move)", t.lower()))
    out = set()
    for cls, words in POLARITY.items():
        if cls == "negate":
            continue
        if w & words:
            out.add(cls)
    if flat:
        ...
        out.discard("up")
        out.discard("down")
    neg = bool(w & POLARITY["negate"]) and not flat
    return out, neg


def polarity_conflict(a, b):
    """True when the two titles cannot be the same bet."""
    pa, na = polarity(a)
    pb, nb = polarity(b)
    # opposite direction classes present
    for x, y in (("up", "down"), ("up", "flat"), ("down", "flat")):
        if (x in pa and y in pb) or (y in pa and x in pb):
            return True
    # one negated, the other not
    if na != nb:
        return True
    return False
...
def match_score(a, b):
    ...
    numshare = len(na & nb) / max(len(na | nb), 1) if (na or nb) else 0.0
    seq = SequenceMatcher(None, a.lower()[:90], b.lower()[:90]).ratio()
    return 0.5 * jac + 0.3 * numshare + 0.2 * seq
...
def find_spreads(pm, kal, min_gap, min_score, min_vol):
    ...
    for p in pm:
        ...
        if p["end"] and best["end"] and p["end"] != best["end"]:
            continue
